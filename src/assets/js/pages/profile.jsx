...
export default function Profile({ user, ratings }) {
  return (
    <>
      <Navbar></Navbar>
              ...
              <div className="bg-white shadow-md rounded-lg overflow-hidden w-full md:w-3/4 mx-auto">
                ...
                <div className="flex items-center justify-between p-6 border-b">
                  <div className='flex flex-row'>
                    <img src={`data:image/jpeg;base64,${user['Photograph']}`} alt="Profile Picture" className="mr-4 w-24 h-24 rounded-full border-2 border-teal-300 shadow-lg"></img>
                    <div className='text-left'>
                      <h2 className="text-2xl font-bold mb-2">{user['Name']}</h2>
                      <p className="text-gray-500">{user['Age']} yaşında</p>
                      <p className="text-gray-500">{user['Location']}</p>
                    </div>
                  </div>
                  <div className='flex flex-col'>
                    <span className="font-extrabold text-4xl text-yellow-500 ">{user['Point']}</span>
                ...
                <div className="flex flex-col items-start justify-center p-6 border-b">
                  ...
                  <p className='mb-4'><span className="font-bold text-green-400 text-left ">{user['Interests']}</span></p>
                  {
                    user['Smoking'] &&
