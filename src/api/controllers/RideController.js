...
module.exports = {
  create: async function(req, res) {
    ...
    try {
      const data = req.body;
      data['Driver'] = req.session.userID;
  ...
  join: async function(req, res) {
    ...
    try {
      ...
      await Ride.updateOne({ id: rideId }).set(ride);
      return res.ok(ride);
    } catch (error) {
      return res.serverError(error);
    }
  ...
  complete: async function(req, res) {
    ...
    try {
      ...
      if (!userId) return res.serverError(500);
      const ride = await Ride.findOne({ id: rideId });
  ...
  comment: async function(req, res) {
    ...
    try {
      ...
      if (!userId) return res.serverError(500);
      const ride = await Ride.findOne({ id: rideId });
      if (ride.Customer != userId) return res.serverError(501);
      ride['isRated'] = true;
      await Ride.updateOne({ id: rideId }).set(ride);
      return res.ok(ride);
    } catch (error) {
